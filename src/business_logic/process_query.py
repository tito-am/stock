...
from src.algo.baseline import Stock_model_Baseline

def create_business_logic():
    data_fetcher = get_last_stock_price
    return BusinessLogic(Stock_model(data_fetcher))

def create_business_logic_baseline():
    data_fetcher = get_last_stock_price
    return BusinessLogic(Stock_model_Baseline(data_fetcher))


class BusinessLogic:

    def __init__(self, model_creator):
        self._root_bucket = 'model_bucket_ycng_228_ftam_jarjarbinks'#'model_bucket_ycng_228'
